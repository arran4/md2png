...
func (r *renderer) renderListItem(li *ast.ListItem, md []byte, level int, marker string, markerLeft, markerRight, contentLeft int) {
	...
	for child := li.FirstChild(); child != nil; child = child.NextSibling() {
		...
		default:
			if child.Kind() == ast.KindTextBlock {
				ensureMarker(startY + int(r.baseSize))
				content := strings.TrimRight(string(child.Text(md)), "\n")
				if content != "" {
					metrics := r.c.drawTokens([]textToken{{text: content, font: r.c.fonts.Regular, size: r.baseSize, color: r.c.th.FG}}, contentLeft, r.c.w-r.c.margin)
					if len(metrics) > 0 && child.NextSibling() != nil {
						r.c.addVSpace(blockSpacing)
					}
				}
			}
