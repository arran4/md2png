...
var (
	// BuiltinSkillLoader is an injectable function that provides the fs.FS for the builtin skill.
	BuiltinSkillLoader func() (fs.FS, error)
)
...
func InstallSkill(source, name string, scope Scope, agentTarget string) error {
	...
	if strings.HasPrefix(source, ".") || strings.HasPrefix(source, "/") || filepath.IsAbs(source) {
		...
		err = InstallFiles(os.DirFS(absSrc), destDir)
		if err != nil {
			return err
		}
		revision = "local"
		...
		err = extractZip(data, "", destDir) // Try root
		if err != nil {
			// If not found in root, try skills/<name>
			err = extractZip(data, "skills/"+name, destDir)
			if err != nil {
				return err
			}
		}
	...
	if err != nil {
		return fmt.Errorf("failed to compute skill digest: %w", err)
	}
