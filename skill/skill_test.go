...
func TestInstallSkillLocal(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(srcDir, "SKILL.md"), []byte("# Test Skill\n"), 0644)

	originalWd, _ := os.Getwd()
	_ = os.Chdir(tmp) // Make sure ScopeProject uses our tmp dir
...
func TestUpdateConflict(t *testing.T) {
	...
	_ = InstallSkill(srcDir, "test_skill", ScopeProject, "")

	destDir := filepath.Join(tmp, ".agents", "skills", "test_skill")

	// Modify the skill locally
	_ = os.WriteFile(filepath.Join(destDir, "SKILL.md"), []byte("# Modified Skill\n"), 0644)
